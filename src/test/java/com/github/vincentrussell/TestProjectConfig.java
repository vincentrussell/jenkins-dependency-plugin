...
public class TestProjectConfig {
    ...
    public File getFile(final String config, final String url) throws IOException {
        File folder = temporaryFolder.newFolder("newProject");
        File newFile = new File(folder, "pom.xml");
        FileUtils.write(newFile, TEXT.replaceAll("REPLACE_ME", config).replaceAll("URL", url), "UTF-8");
        return newFile;
    }
}
